...
The project has 235 Dart tests (page-column geometry, drag-and-drop placement, dirty-state policy, the close-everything flow, cut/copy/paste, protected documents) and 26 native tests (`macos/RunnerTests`, toolbar item management, document-window placement, PDF encryption detection). Everything else is uncovered, which remains the largest known risk (CODE_REVIEW §5.3). The following is the target state, and applies to code you add or change:
